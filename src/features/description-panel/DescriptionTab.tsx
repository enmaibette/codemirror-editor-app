...
export function DescriptionTab({ challenge }: DescriptionTabProps) {
  ...
  return (
    <div className="p-4 space-y-5">
      ...
      <div className="text-sm leading-relaxed text-[var(--text)] space-y-4">
        <ReactMarkdown
          components={{
            h2: ({ children }) => (
              <h3 className="text-sm font-semibold text-[var(--text)] mt-4">{children}</h3>
            ),
            ul: ({ children }) => (
            ...
            code({ className, children }) {
              ...
              return lang === 'python' ? (
                ...
                <code className="rounded bg-[var(--elevated)] px-1 py-0.5 text-[0.9em]">
                  {children}
                </code>
              );
            },
